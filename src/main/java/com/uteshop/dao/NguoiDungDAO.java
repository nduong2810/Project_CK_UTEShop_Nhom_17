...
public class NguoiDungDAO {
	...
	public boolean save(NguoiDung user) {
		...
		try (Connection conn = DBConnect.getConnection();
			...
			VietnameseEncodingUtil.setVietnameseParameter(ps, 4,
					VietnameseEncodingUtil.prepareVietnameseText(user.getHoTen()));
			VietnameseEncodingUtil.setVietnameseParameter(ps, 5, user.getSoDienThoai());
			VietnameseEncodingUtil.setVietnameseParameter(ps, 6,
					VietnameseEncodingUtil.prepareVietnameseText(user.getDiaChi()));
	...
	public boolean update(NguoiDung user) {
		...
		try (Connection conn = DBConnect.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			// Use VietnameseEncodingUtil for proper Unicode handling
			VietnameseEncodingUtil.setVietnameseParameter(ps, 1,
					VietnameseEncodingUtil.prepareVietnameseText(user.getHoTen()));
	...
	public int countAllActive() {
		String sql = "SELECT COUNT(*) FROM NguoiDung WHERE TrangThai = 1";
		try (Connection conn = DBConnect.getConnection();
				PreparedStatement ps = conn.prepareStatement(sql);
				ResultSet rs = ps.executeQuery()) {
			rs.next();
	...
	public int countByRole(String vaiTro, String q) {
		StringBuilder sb = new StringBuilder("SELECT COUNT(*) FROM NguoiDung WHERE 1=1");
		List<Object> params = new ArrayList<>();
		if (vaiTro != null && !vaiTro.isBlank()) {
			sb.append(" AND VaiTro=?");
			params.add(vaiTro);
		}
		if (q != null && !q.isBlank()) {
			sb.append(" AND (HoTen LIKE ? OR Email LIKE ? OR TenDangNhap LIKE ? OR SoDienThoai LIKE ?)");
			String like = "%" + q.trim() + "%";
			params.add(like);
			params.add(like);
			params.add(like);
		...
		try (Connection conn = DBConnect.getConnection(); PreparedStatement ps = conn.prepareStatement(sb.toString())) {
			int i = 1;
			for (Object p : params)
				ps.setNString(i++, p.toString());
			try (ResultSet rs = ps.executeQuery()) {
				rs.next();
				return rs.getInt(1);
			}
